 ...
 */
'use strict';
cBoard.controller('datasetAlertCtrl', function ($scope, $stateParams, $http, $uibModal, dataService, ModalUtils, updateService, $filter, chartService) {
    ...
    $scope.loadData = function () {
        $scope.loading = true;
        console.log($scope.curWidget.query)
...
// };
    	...
    	$http.get("/dashboard/getDatasourceList.do").success(function (response) {
            $scope.datasourceList = response;
            	for(var i=0;i<$scope.datasourceList.length;i++){
            		if($scope.datasourceList[i].type=="presto"){
            			$scope.datasource=$scope.datasourceList[i];
            		}
            	}
        });
    });
    ...
    $scope.aceLoad = function(_editor) {
		   if(_editor.session.doc.getValue().length == 0){
			   _editor.session.doc.setValue($stateParams.query);
			   $scope.curWidget.query.sql=$stateParams.query;
			   
			   loadData();
		   }
	  };
	  $scope.aceChange = function(_editor) {
		   if(_editor[1].session.doc.getValue().length == 0){
			   _editor[1].session.doc.setValue($stateParams.query);
			   $scope.curWidget.query.sql=$stateParams.query;
			   loadData();
		   }
	  };
   $scope.aceBlur = function(_editor) {
		   if(_editor[0].session.doc.getValue().length == 0){
			   _editor[0].session.doc.setValue($stateParams.query);
			   $scope.curWidget.query.sql=$stateParams.query;
			   loadData();
		   }
	  };
